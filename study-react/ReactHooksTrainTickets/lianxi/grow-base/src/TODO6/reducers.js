...
function combineReducers(reducers) {
    ...
    return function reducer(state,action) {
       const changed = {};
        ...
        console.log(changed,'kanreducers文件的changed值')
...
const reducers = {//作用就是让对应的sttae值都只代表对应的todos值或increment值，而不是下面之前版本reducer函数里的state代表所有值
    todos(state, action) {//传的2参就是 传state是全局状态的todos对应的state值。action仅仅改这个传进来的state值，
                          //这样就所有state值分散开来，与action一一对应。
        const {type,payload} = action;
        switch(type) {
            case 'set':
                return {
                    //这时的payload就是个完整todos数组
                    todos:payload,
                }
            case 'add':
                return  [
