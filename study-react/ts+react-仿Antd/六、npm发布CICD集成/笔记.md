...
- 手动操作比较繁琐，我们先要在本地运行npm build-storybook生成静态文件，然后新建个ghpages的分支，把文件都拷贝过去然后在push上去----但使用travis我们可以轻松实现每次push以后自动部署
- 参照travis tutorial自动部署那一块文档，就是往.travis.yml文件添加些deploy属性配置拷贝过来就好
- 得额外加下scripts属性，指定执行哪个脚本，默认执行npm test命令，现在我们写成npm run script: build-storybook生成静态文件命令
- 然后github_token是环境变量修改成小写都可以
- 然后再加个local_dir：storybook-static  即告诉你我要上传哪个文件夹里的内容默认是build文件夹
...
- 最后我们要配置下github_token,得去github官网操作
  - 去github官网个人设置里Settings/Developer settings/Personal access token页面生成个token即可，生层以后把token拷贝过来去travis你工作台页面的Moreoptions/Settings/环境变量处添加个环境变量，变量名称一定要跟你yml文件里的名字对应
- 最后addcommitpush部署成功了

**访问站点地址：**<https://github用户名.github.io/项目名/？path=/story/welcome-page--welcome>
